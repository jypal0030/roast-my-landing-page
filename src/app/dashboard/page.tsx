      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {roasts.map((roast) => (
                <Link key={roast.id} href={`/roast/${roast.id}`} className="flex items-center justify-between rounded-xl border border-ash-700 bg-ash-800/50 p-4 hover:border-ash-600 transition-all">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-ash-700 bg-ash-800/50 p-5">
            ...
            <div className="space-y-2">
              <Link href="/" className="block w-full rounded-lg bg-fire-500 px-4 py-2 text-sm font-bold text-white hover:bg-fire-600 text-center"><Flame className="h-4 w-4 inline mr-1" /> Roast a Website</Link>
              {(user.plan === "free" || !user.plan) && <Link href="/pricing" className="block w-full rounded-lg border border-ash-600 px-4 py-2 text-sm font-medium text-ash-300 hover:bg-ash-700 text-center"><Crown className="h-4 w-4 inline mr-1" /> Upgrade Plan</Link>}
            </div>
          </div>
        </div>
      </div>
